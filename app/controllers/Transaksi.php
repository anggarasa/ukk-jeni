...
class Transaksi extends Controller {
    ...
    public function detail($id = null) {
        if ($id) {
            ...
            $detail = $transaksiModel->getDetailTransaksi($id);
    ...
    public function checkStock() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($postData['produk_id']) && isset($postData['quantity'])) {
                $produkModel = $this->model('ProdukModel');
                $produk = $produkModel->getById($postData['produk_id']);

                if ($produk && $produk['stok'] >= $postData['quantity']) {
                    echo json_encode(['status' => 'success', 'available' => true]);
                } else {
                    echo json_encode([
                        'status' => 'error',
                        'available' => false,
                        'message' => 'Stok tidak mencukupi. Tersedia: ' . ($produk ? $produk['stok'] : 0)
                    ]);
                }
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Data tidak lengkap']);
